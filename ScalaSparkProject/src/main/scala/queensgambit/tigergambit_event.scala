...
object tigergambit_event {
  def main(args: Array[String]): Unit = {
    ...
    //Default Settings
    val conf = new SparkConf().setAppName("TigerGambit_Event").setMaster("local[2]")
    val sc = new SparkContext(conf)
    sc.hadoopConfiguration.set("textinputformat.record.delimiter", "[Event")

    //Delete File When Start
    FileUtils.deleteQuietly(new File("output_event_final"))
    ...
    try{
      ...
      val mapData = getFile.filter(line => line.contains("Queen's Gambit")).map(line =>{
        ...
        if(format.parse(utc).compareTo(targetDate) < 0) {
          ...
          else if(event.contains("Rated UltraBullet tournament")){
            ("Before: Rated UltraBullet tournament", 1)
          }
          else {
            ("Before:" + event, 1)
          }
        ...
        //Else will be after the show
          ...
          if(event.contains("Rated Blitz tournament")){
            ...
            ("After: Rated UltraBullet tournament", 1)
          }
          else {
            ("After:" + event, 1)
          }
      ...
      finalData.saveAsTextFile("output_event_final")
    }
    catch{
      case ex: Exception =>{
        println("Error!")
